...
local isChopping = false -- Prevents multiple chop attempts at once
local areaZone = nil
local areaBlip = nil
local chopVehicle = 0
    ...
    CreateThread(function()
        ...
        while waitingForAnswer do
            ...
            if IsControlJustPressed(0, 38) then 
                ...
                -- D. SUBTITLES THREAD (Syncs with your timestamps)
                CreateThread(function()
                    -- 0-2s: Hehe, yeah, my G.
...
local function DrawMissionHUD()
    CreateThread(function()
        while targetPlate do -- Checks at start of loop
            Wait(0)
            
            -- FIX: Stop immediately if it became nil during the Wait(0)
            ...
            BeginTextCommandDisplayText("STRING")
            -- Added tostring() just in case model is somehow nil, though unlikely
            ...
            BeginTextCommandDisplayText("STRING")
            -- The crash happened here previously
...
local function ChopVehicleAction(veh)
        ...
        if boneIndex ~= -1 then
            ...
            while true do
                ...
                if #(GetEntityCoords(ped) - coords) < 1.5 and IsControlJustReleased(1, 38) then 
                    -- 1. Auto Face the Part
                    TaskTurnPedToFaceCoord(ped, coords.x, coords.y, coords.z, 1500)
                    Wait(1500) -- Wait for turn to finish
                    break 
                end
            end

            -- Open door if applicable (not for wheels)
    ...
    -- 2a. Subtitle - Forces it to display by running it in a thread to avoid being cleared instantly
    CreateThread(function()
        Subtitle("Then whose car was it?", 5000)
    end)
    Wait(5000) -- Wait for subtitle to finish reading
    ...
    QBCore.Functions.SpawnVehicle("banshee2", function(banshee)
        ...
        -- Set Destination to C1 (Beginning Marker)
        ...
        SetBlipColour(destBlip, 5) -- Yellow
        ...
        CreateThread(function()
            ...
            while active do
                Wait(0) -- CRITICAL: Must be 0 to draw text/marker every frame
                ...
                if dist < 5.0 then
                    ...
                    Wait(1500) -- The requested 1.5s wait
...
local function StartScrapyardInteraction()
    ...
    -- 2. START LONG FADE OUT
    DoScreenFadeOut(3000) -- 3 second fade out
    while not IsScreenFadedOut() do Wait(10) end
    
    -- 3. SPAWN PED JOSEF (While screen is black)
    local pedModel = `ig_josef`
    RequestModel(pedModel)
    while not HasModelLoaded(pedModel) do Wait(0) end
    
    -- Ped Coords: Fixed Z by subtracting 1.0 (48.15 -> 47.15)
    local josef = CreatePed(4, pedModel, 2339.44, 3051.93, 47.15, 273.39, false, true)
    
    -- Safety snap to ground & Persistence
    FreezeEntityPosition(josef, true)
    SetEntityInvincible(josef, true)
    SetBlockingOfNonTemporaryEvents(josef, true)
    
    -- CRITICAL: This keeps him spawned!
    SetEntityAsMissionEntity(josef, true, true) 

    -- 4. CUTSCENE SETUP (Placeholder/Wait)
    Wait(2000) 
    
    -- 5. FADE IN
    DoScreenFadeIn(3000) -- 3 second fade in
    while not IsScreenFadedIn() do Wait(10) end

    -- 6. START CHOP SHOP MISSION
...
RegisterCommand("testcall", function(source, args)
    ...
    local duration = tonumber(args[2]) or 29500 -- Default to 29.5s (Josef's length)
    
    if not soundName then 
        QBCore.Functions.Notify("Usage: /testcall [filename] [duration]", "error")
        return 
    end
    ...
    CreateThread(function()
        ...
        while waitingForAnswer do
            ...
            SetTextComponentFormat("STRING")
            AddTextComponentString("Incoming Call: ~b~Unknown~n~~w~Press ~g~[E]~w~ to Answer")
            DisplayHelpTextFromStringLabel(0, 0, 1, -1)
            ...
            if IsControlJustPressed(0, 38) then 
                ...
                TriggerServerEvent('InteractSound_SV:PlayOnSource', soundName, 1.0)
                ...
                -- B. SPECIFIC SUBTITLES FOR JOSEF
                if soundName == "josef_call" then
                    CreateThread(function()
                        Subtitle("Hehe, yeah, my G.", 2000)
                        Wait(2000)
                        Subtitle("What's good!", 1000)
